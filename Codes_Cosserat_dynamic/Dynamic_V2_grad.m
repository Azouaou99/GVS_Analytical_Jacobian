function [res,Grad] = Dynamic_V2_grad(var,q0,qd0,Param)
...
C=Param.L/(Param.n_X)*((fC10+fC1n)/2 + SC1);
...
Grad= M*(Param.dt)^(-2)*eye(Param.n*Param.na)+(C+Deps)*(Param.dt)^(-1)*eye(Param.n*Param.na)+Mq+Cq+Keps+Feq;
%Grad=Mq+Cq+Keps+Feq;
res=M*qdd1-Hq*Param.Forces_Tendons+Fe+C*qd1+Keps*q1+Deps*qd1;%2nd derivative of q
